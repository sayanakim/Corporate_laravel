...
<div id="content-single" class="content group">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In:
                    <a href="{{ route('articlesCat', ['cat_alias'=>$article->category->alias]) }}"
                       title="View all posts in {{ $article->category->title }}" rel="category tag">{{ $article->category->title }}</a>,
            <p class="comments"><span><a href="#comments" title="">{{ count($article->comments) ? count($article->comments) : '0' }}
                    {{ Lang::choice('ru.comments', count($article->comments)) }}</a></span></p>
        </div>
    ...
    {{--        формирование древовидного комментария--}}
            ...
            {{-- если пользователь зарегистрирован--}}
                @if(!\Illuminate\Support\Facades\Auth::check())
                    ...
                    <p class="comment-form-url"><label for="url">Веб-сайт</label>
                        <input id="url" name="site" type="text" value="" size="30" /></p>
                @endif
